...
const validationSchema = Yup.object().shape({
  ...
  EPF_ID: Yup.string().required(),
  SI_ID: Yup.string().required(),
});
    ...
    await api
      ...
      .catch(err => {
        ...
        if (err.request.status === 400) {
          setDone('Employee already exist');
          const timer = setTimeout(() => {
            setModal(false);
          }, 2000);
          return () => clearTimeout(timer);
  ...
  return (
    <View style={styles.MainContainer}>
      <View style={styles.HeaderContainer}>
        <Text style={styles.HeaderContainerText}>Register new Employee</Text>
      </View>
      <KeyboardAvoidingView
          ...
          <Formik
           ...
           initialValues={{
            ...
            onSubmit={(values ,{ resetForm}) => (
              EmployeeRegister(values , resetForm), setModal(true), setDone('Please Wait')
            )}>
            {({
              handleChange,
              handleBlur,
